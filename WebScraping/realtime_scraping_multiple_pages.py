...
def find_jobs():
    base_url =("https://www.timesjobs.com/candidate/job-search.html?searchType=personalizedSearch&from=submit&searchTextSrc=as&searchTextText=Python&txtKeywords=Python&txtLocation=&sequence=")
 
    new_jobs = []  # List to store new jobs temporarily
    #This scraps limited number of pages. For scraping all available pages. See the commented code at the last
    for page in range(1,3):
        print(f"Scraping page {page}...")
        html_text = requests.get(base_url + str(page)).text
        soup=BeautifulSoup(html_text,'lxml')
        job_cards=soup.find_all('li', class_='clearfix job-bx wht-shd-bx')

        if not job_cards:  # Stop if no job cards are found on the page
...
def find_jobs():
    base_url = (
        ...
        "&from=submit&searchTextSrc=as&searchTextText=Python&txtKeywords=Python&txtLocation=&sequence="
    )
